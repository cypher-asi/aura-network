...
pub async fn delete(pool: &PgPool, org_id: Uuid) -> Result<(), AppError> {
    ...
    if project_count > 0 {
        return Err(AppError::BadRequest(
            "Cannot delete organization with existing projects. Delete all projects first.".into(),
        ));
    }
    ...
    if agent_count > 0 {
        return Err(AppError::BadRequest(
            "Cannot delete organization with existing agents. Delete all agents first.".into(),
        ));
    }

    let mut tx = pool.begin().await?;

    // Clean up invites and members, then delete org
